...
def split_text(x):
    ...
    if pd.isna(text):
        return np.nan
    sections = text.split('\\n\\n')
    keys = {'Verse 1': np.nan, 'Verse 2': np.nan,
            'Verse 3': np.nan, 'Verse 4': np.nan, 'Chorus': np.nan}
    lyrics = str()
    single_text = []
    res = {}
    for s in sections:
...
pdf['single_text'] = pdf['Poem'].apply(lambda x: ' \n '.join(
    [l.lower().strip().translate(translator) for l in x.splitlines() if len(l) > 0]))
pdf.head()
...
def extract_text(text):
    global text_as_list
    text_as_list += [w for w in text.split(' ')
                     if w.strip() != '' or w == '\n']
...
print('Belirli bir sıklıktan daha az {} görülen kelimeler: {}'.format(
    MIN_FREQUENCY, len(uncommon_words)))
print('Belirli bir sıklıktan daha çok {} görülen kelimeler: {}'.format(
    MIN_FREQUENCY, len(words)))
valid_seqs = []
end_seq_words = []
...
print('Belirli bir uzunluktaki {} geçerli diziler: {}'.format(
    MIN_SEQ, len(valid_seqs)))

# Verilerimizi train ve test olarak ayırma
X_train, X_test, y_train, y_test = train_test_split(
    valid_seqs, end_seq_words, test_size=0.02, random_state=42)

# Çok önemli.
# Yüksek yoğunlukta veri ilşe çalıştığımız için ram miktarımız yeterli gelmeme ihtimaline sahiptir.
# "Out of memory" hatası almamak için bir data generator yazacağız.
# Fonksiyonlar kerastan alınmıştır!!!
...
file_path = "./save/lyrics_gen_model.h5"
checkpoint = ModelCheckpoint(
    file_path, monitor='val_accuracy', save_best_only=True)
